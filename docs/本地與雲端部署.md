...
本文件整理 **cales-breathe-v2** 的部署方式：本機以 Docker Compose 為主線，雲端以 **Vercel（前端）＋ Google Cloud Run（後端）** 為目前程式庫設定的主線；細節仍以下列深讀文件為準。

| 情境 | 主要入口 |
|------|----------|
| 本機整合（Postgres + API + 前端靜態 + NGINX） | 根目錄 [`README.md`](../README.md)、[`docs/DOCKER_DEVELOPMENT_PLAN.md`](DOCKER_DEVELOPMENT_PLAN.md) |
